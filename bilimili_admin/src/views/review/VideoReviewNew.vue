                ...
                <div class="video-table-wrap">
                    <div class="v-table" v-loading="loading">
                        <div class="table-title">
                            <div class="titleBar">
                                <div class="title-item waiting" :class="videoStatus === 0 ? 'active' : ''" @click="handleChange(0)">待审核</div>
                                <div class="title-item passed" :class="videoStatus === 1 ? 'active' : ''" @click="handleChange(1)">已过审</div>
                                <div class="title-item banned" :class="videoStatus === 2 ? 'active' : ''" @click="handleChange(2)">未过审</div>
                            </div>
                        ...
                        <div class="v-table__wrapper">
                            <table>
                                ...
                                <tr v-for="(item, index) in videos" :key="index">
                                    <td>{{ item.video.vid }}</td>
                                    <td><img :src="item.video.coverUrl" class="videoCover" alt=""></td>
                                    <td>{{ item.video.title }}</td>
                                    <td>{{ item.user.nickname }}</td>
                                    <td>
                                        <span class="category" style="color: #1b3d94;">{{ item.category.mcName }}</span> /
                                        <span class="category" style="color: #3884da;">{{ item.category.scName }}</span>
                                    </td>
                                    <td>{{ item.video.uploadDate }}</td>
                                    <td>
                                            <span
                                                class="detail-btn"
                                                @click="openNewPage({
                            ...
                            <div class="blank" v-if="!loading && videos.length == 0">
                                <span>尚无任何数据</span>
                            </div>
                        </div>
